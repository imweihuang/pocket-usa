...
// View of median age chart
class AgeCardView: UIView {
    ...
    init(_ locationID: String, bookmark: Bool) {
        ...
        if bookmark {
         ...
         super.init(frame: CGRect(x: 375, y: 0, width: 375, height: 400 - 15))
        }
        // Initialize bookmark
        initBookmarkButton()
    ...
    // Get medium age
    func getMediumAge(completion: @escaping () -> Void) {
        URLCache.shared = URLCache(memoryCapacity: 0, diskCapacity: 0, diskPath: nil)
    ...
    func parseDataFeed(completion: @escaping () -> Void) {
        ...
        for subFeed in dataFeed {
            ...
            // Retrieve age figure
            if let age = subFeed[4] as? NSNumber {
                medianAges.append(age as! Double)
                print("### Age: \(age)")
            } else {print("### ERROR converting age")}
        }
        // Back to the main thread
        DispatchQueue.main.async {
            completion()
    ...
    func initDescription() {
        ...
        squareLabel.textColor = ColorPalette.deepOrange500
        ...
        if (locationID == "01000US") {
            textLabel.text = "This chart shows the median age of United States."
        } else {
            textLabel.text = "This chart shows the median age of \(locationNames[locationIDs.index(of: locationID)!]) compared to its parent locations."
        }
...
extension AgeCardView {
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        // Initial age label
        let initialAgeLabel = UILabel(frame: CGRect(x: Double(bottomLeftConor.x) - 15, y: Double(bottomLeftConor.y) + 2, width: 30, height: 14))
        initialAgeLabel.textColor = ColorPalette.lightGrayA8
        initialAgeLabel.font = UIFont(name: initialAgeLabel.font.fontName, size: 10)
        initialAgeLabel.textAlignment = .center
        initialAgeLabel.numberOfLines = 1
        initialAgeLabel.text = "0"
        self.addSubview(initialAgeLabel)
    }
    
    func drawBars() {
        print("### Location Names: \(self.locationNames)")
        if (medianAges.count > 0) {
            let barLayer = CAShapeLayer()
            let targetBarLayer = CAShapeLayer()
            let barPath = UIBezierPath()
            let targetBarPath = UIBezierPath()
            let barWidth = (Double(bottomLeftConor.y - topLeftConor.y) - barSpacing) / Double(medianAges.count) - barSpacing
            var newEndX: Double!
            var newStartY: Double!
            // Set layer colors
            barLayer.strokeColor = ColorPalette.lightGrayA8.cgColor
            targetBarLayer.strokeColor = ColorPalette.deepOrange500A8.cgColor
            ...
            var index = 0.0 // index of current age
            // Log
            ...
            // Draw guiding lines
            drawGuidingLines(medianAges.max()!)
            // Create age bars paths and labels
            for age in medianAges {
                print("### Bar \(index) - age, max age: \(age), \(medianAges.max()!)")
                newEndX = Double(topLeftConor.x) + Double(age / medianAges.max()!) * (Double(topRightConor.x - topLeftConor.x) /* - barSpacing */ /*- topLeftConor.x */)
    ...
    // Draw guiding lines
    func drawGuidingLines(_ maxAge: Double) {
        let guideLinesLayer = CAShapeLayer()
        let guideLinePath = UIBezierPath()
        let lineCount = Int(floor(medianAges.max()! / 5))
        ...
        for var i in 1...lineCount {
            let newX = Double(topLeftConor.x) + Double(i * 5)/maxAge * (Double(topRightConor.x - topLeftConor.x) /* - barSpacing */)
            // Craete Line
            ...
            // Create label
            if (i % 2 == 0) {
                let ageLabel = UILabel(frame: CGRect(x: newX - 15, y: Double(bottomLeftConor.y) + 2, width: 30, height: 14))
                ageLabel.textColor = ColorPalette.lightGrayA8
                ageLabel.font = UIFont(name: ageLabel.font.fontName, size: 10)
                ageLabel.textAlignment = .center
                ageLabel.numberOfLines = 1
                ageLabel.text = String(i * 5)
